...
// Same lib/services.ts data used by the homepage shortlist and the individual
// service pages, so pricing and descriptions can never drift between pages.
const FEATURED_SLUG = "airbnb";
const allServices = getAllServiceSlugs().map((slug) => SERVICES[slug]);
    ...
    <main className="min-h-screen bg-brand-bg">
      ...
      <section className="py-16">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 flex flex-col gap-6">
          {allServices.map((service, index) => {
            const { slug, icon: Icon, title, description, price, priceLabel, bookable, highlights } = service;
            const featured = slug === FEATURED_SLUG;
            return (
              <Reveal key={slug} delay={(index % 3) * 0.08}>
              <div
                className={`rounded-3xl border p-8 flex flex-col sm:flex-row gap-6 items-start justify-between transition-all duration-200 hover:shadow-lg hover:-translate-y-1 motion-reduce:hover:translate-y-0 ${featured
                  ? "bg-gradient-to-br from-brand to-brand-accent border-brand-accent shadow-lg shadow-brand-accent/20"
                  : "bg-brand-surface border-brand-border hover:border-brand-accent/40"
                  }`}
              >
                <div className="flex gap-5 items-start flex-1">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      {slug === "move" && <span className="text-[10px] font-bold uppercase tracking-wider bg-brand/10 text-brand px-2 py-0.5 rounded-full">Our Specialty</span>}
                    </div>
                    <p className={`text-sm leading-relaxed max-w-xl ${featured ? "text-white/75" : "text-brand-muted"}`}>{description}</p>
                    <ul className="flex flex-wrap gap-2 mt-1">
                      {highlights.map(({ label }) => (
                        <li key={label} className="flex items-center gap-1.5">
                          <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 ${featured ? "text-white/70" : "text-brand-accent-dark"}`} />
                          <span className={`text-xs ${featured ? "text-white/80" : "text-brand-text"}`}>{label}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 justify-end">
                    {bookable ? (
                      <Button asChild className="bg-brand-accent hover:bg-brand-accent-dark text-white font-semibold gap-1.5">
                        <Link href={bookingWithService(slug)}>Book Now <ArrowRight className="w-3.5 h-3.5" /></Link>
                      </Button>
                    ) : (
                      <Button
                        asChild
                      ...
                      >
                        <Link href={ROUTES.QUOTE}>Get a Free Quote <ArrowRight className="w-3.5 h-3.5" /></Link>
                      </Button>
                    )}
                    <Button
                      asChild
                    ...
                    >
                      <Link href={`${ROUTES.SERVICES}/${slug}`}>Learn More <ArrowRight className="w-3.5 h-3.5" /></Link>
                    </Button>
                  </div>
                </div>
              </div>
      ...
      <section className="pb-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <Reveal className="bg-gradient-to-r from-[#0C1A2E] to-[#0F766E] rounded-3xl px-8 py-10 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
